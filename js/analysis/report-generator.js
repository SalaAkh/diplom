...
class ReportGenerator {
    ...
    generateHTMLReport(data, format = 'html') {
        ...
        const pdfWidth = '1000px';

        // ==========================================
        // НАСТРОЙКИ PDF (PDF SETTINGS) - EDIT HERE
        // ==========================================
        const pdfLayout = {
            width: '900px',             // Ширина контента (Content Width)
            padding: '40px 40px 40px 40px',            // Отступы внутри (Padding): Top Right Bottom Left
            marginLeft: '580px'         // Отступ слева от края листа (Left Margin)
        };
        // ==========================================

        const printWrapperStyle = format === 'pdf'
            // Убрали 'margin: 0 auto', поставили margin-left: 0 для жесткой привязки к левому краю
            ? `style="margin: 0 0 0 ${pdfLayout.marginLeft} !important; width: ${pdfLayout.width} !important; max-width: 100% !important; display: block !important; padding: ${pdfLayout.padding} !important; box-sizing: border-box !important; background: #fff !important; overflow: hidden !important;"`
            : '';

        const bodyStyle = format === 'pdf' ?
            `width: ${pdfLayout.width}; margin: 0 0 0 ${pdfLayout.marginLeft}; padding: 0; background: #ffffff; box-sizing: border-box;` :
...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        /* Override global justify for header text */
        .header p, .header h1 {
        ...
        .summary-section p {
            ...
            font-size: 15px;
            line-height: 1.8;
            text-align: justify; /* Secondary text justified per user request */
            text-indent: 1.5em;
            margin: 0;
        }
...
<body>
    ...
    <div class="footer" style="padding-bottom: 20px; margin-top: 30px; clear: both; text-align: center;">
        <p style="margin-bottom: 15px; font-weight: 700; color: #1e293b; font-size: 12px; text-transform: uppercase; letter-spacing: 1px; text-align: center !important;">
        ...
        <div style="background: #f8fafc; padding: 20px; border-radius: 16px; border: 2px solid #e2e8f0; display: block; max-width: 550px; margin: 0 auto; text-align: center;">
            <p style="font-weight: 800; color: #0f172a; font-size: 16px; margin: 0 0 5px 0; text-align: center !important;">
            ...
            <p style="margin: 0; font-size: 12px; color: #475569; font-weight: 600; text-align: center !important;">
    ...
    async generatePDFReport(data) {
        ...
        const opt = {
            ...
            html2canvas: {
                ...
                allowTaint: false,
                windowWidth: 900, // Matched with CSS width
                scrollY: 0,
                scrollX: 0
            },
